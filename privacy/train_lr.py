...
def train_vanilla(n_splits, n_repeats, scaler_type, l1, C,
    ...
    for comb in all_combs:
        ...
        model_type, param_dict = create_lr_model(l1, C)
        ...
        for i, (train_idx, _) in enumerate(kf.split(x, y_z)):
            ...
            resc_params_dir = os.path.join(comb_dir, f'{C}_{l1}_{i}_resc_params.pkl')
...
def train_average(n_splits, n_repeats, n_models, scaler_type, l1, C,
                  all_data, featuresNA, phenoNA, datasets_ids):
    x = all_data[featuresNA].values
    y = all_data[phenoNA].values
    z = all_data['DatasetNum'].values
...
if __name__ == '__main__':
    ...
    if options['vanilla']:
        if len(args) == 5:
            ...
            l1 = float(args[3])       # [0.0, 0.5, 1.0]  -> 1.0
            C = float(args[4])        # [0.1, 1.0, 10.0] -> 0.1
        else:
    ...
    if options['average']:
        if len(args) == 6:
            ...
            l1 = float(args[4])       # [0.0, 0.5, 1.0]  -> 1.0
            C = float(args[5])        # [0.1, 1.0, 10.0] -> 0.1
        else:
